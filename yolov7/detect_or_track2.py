...
def detect(imgs, img3, streamTable, save_img=False):
# def detect(save_img=False):
    # total_cam = len(opt.source)
    # sources = ["0","1"]
            ...
            if webcam:
                ...
                with open(source) as f:
                    for i in f.readlines():    
                        cap = cv2.VideoCapture(int(i.rstrip()))
                        ret, frame = cap.read()
                ret = False
                while ret != True:
                    ret, frame = cap.read()
            ...
            for path, img, im0s, vid_cap in dataset:
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(det):
                        ...
                        detectface(im0, [bbox_xyxy], img3, streamTable, value["camera_id"], id)
                        # detectface(im0, [bbox_xyxy], id)
                        im0 = draw_boxes(im0, bbox_xyxy, identities, categories, confidences, names, colors)
                        
                        # for i in range(len(path)):
                        imgs[int(s[0])].image(im0)
...
def run(imgs, img3, streamTable, sourceST):
    global opt, sort_tracker
